...
import {List, message, Space, Tooltip} from 'antd';
...
export const MyRecipes = () => {
    ...
    const Description = ({text, time, rating}: {
        text: string,
        time: number,
        rating: { rate: number, reviews: number }
    }) => {
        return (
    ...
    async function handleDelete(id: number) {
        try {
            ...
            if (response.status != 200) {
                message.error('Failed to delete your recipe')
            } else {
                setRecipes(recipesTab.filter(item => item.id !== id));
                message.success('Recipe sucesfully deleted');
            }
        } catch (error) {
            console.log(error);
        }
